...
optButtons.push({
    ...
    click: (row: TableRow) => {
        ElMessageBox.prompt(t('sms.template.Please enter the receiver mobile'), t('sms.template.test_send'), {
            confirmButtonText: t('sms.config.send out'),
            ...
            beforeClose: (action, instance, done) => {
                if (action === 'confirm') {
                    ...
                    instance.confirmButtonText = t('sms.config.Sending')
...
const baTable = new baTableClass(new baTableApi('/admin/sms.template/'), {
  pk: 'id',
  column: [
