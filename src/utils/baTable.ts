...
export default class baTable {
    ...
    constructor(api: baTableApi, table: {
        dblClickNotEditColumn: undefined[];
        column: ({ type: string; align: string; operator: boolean } | {
            prop: string;
            label: string;
            align: string;
            render: string;
            ...
            render: string;
            operator: string
        } | { buttons: OptButton[]; width: number; label: string; align: string; render: string; operator: boolean })[];
        pk: string
    ...
    // 编辑
    requestUpdate = (id: string) => {
        if (this.runBefore('requestUpdate', { id }) === false) return
        this.form.loading = true
        this.form.items = {}
        return this.api
            .edit({
            ...
            .then((res) => {
                this.form.items = res.data.row
                this.runAfter('requestUpdate', { res })
            })
            .catch((err) => {
                this.toggleForm()
                this.runAfter('requestUpdate', { err })
            })
            .finally(() => {
                this.form.loading = false
            })
    ...
    toggleForm = (operate = '', operateIds: string[] = []) => {
        ...
        if (operate == 'Edit') {
            ...
            this.requestUpdate(operateIds[0])
    ...
    onTableAction = (event: string, data: anyObj) => {
        ...
        const actionFun = new Map([
            ...
            [
                'update',
                () => {
                    this.toggleForm('Update', [data.row[this.table.pk!]])
                },
            ],
            [
                'delete',
